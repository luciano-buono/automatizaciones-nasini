...
name_of_file = 'CLOSEOPCIONES-0062-20220311.DAT'
try:
...
def split_file():
    splitLen = int(input("Lineas por archivo? (Default2000): ") or "2000") # X lines per file
    outputBase = f'{out_file_name}-sorted' # output.1.txt, output.2.txt, etc.

    # This is shorthand and not friendly with memory
    # on very large files (Sean Cavanagh), but it works.
    inputFile = open(f'{out_file_name}-sorted{file_extension}', 'r').read().split('\n')

    at = 1
    for lines in range(0, len(inputFile), splitLen):
        # First, get the list slice
        outputData = inputFile[lines:lines+splitLen]
